...
ControllerAll = false
local ListSudos ={Sudo_Id,5984712101,5298947457}
...
if text and Redis:get(DyLer..'lock:telethon'..msg.chat_id) and not msg.Special then 
list = {".تكرار",".مكرر",".كرر",".مؤقت",".موقت","$تكرار","$تكرار","$موقت","$مؤقت",".وقتي",".وقتي","$وقتي","&تكرار","$توقيت","$وقت",".تكرر",".توقيت"}
for k,v in pairs(list) do
if string.find(text,v) ~= nil then
LuaTele.setChatMemberStatus(msg.chat_id,{[1]= msg.id})
return false
end
end
end
...
if text == 'قفل التليثون'  then
if not msg.Admin then
return send(msg_chat_id,msg_id,'\n*↯︙ هذا الامر يخص  '..Controller_Num(7)..' * ',"md",true)  
end
Redis:set(DyLer..'lock:telethon'..msg.chat_id,true) 
send(msg_chat_id,msg_id,Reply_Status(msg.sender.user_id,"↯︙ تم قفـل التليثون").Lock,"md",true)  
end
...
if text == 'فتح التليثون' then
if not msg.Admin then
return send(msg_chat_id,msg_id,'\n*↯︙ هذا الامر يخص  '..Controller_Num(7)..' * ',"md",true)  
end
Redis:del(DyLer..'lock:telethon'..msg.chat_id) 
send(msg_chat_id,msg_id,Reply_Status(msg.sender.user_id,"↯︙ تم فتح التليثون").unLock,"md",true)  
end
...
return send(msg_chat_id,msg_id,"[(Source Dev)](tg://user?id=5984712101)","md",true, false, false, true, reply_markup)
end
...
local CmdStart = '*\n↯︙ اهلا بك عزيزي انا بوت '..(Redis:get(DyLer.."Name:Bot") or "دايلر")..
